...
- Memory: compact task state and outcomes in `chay-memory/task_note.json`,
  `chay-memory/context_package.json`, `chay-memory/plan_ledger.json`, and
...
The worker reads the compact note, selected chay-memory refs, and the files in
`allowed_files`. It should not read raw logs, full prompts, or
the entire repository.

## Runtime Rule

Only Chạy Runtime writes `chay-memory/plan_ledger.json`. Dispatch updates the ledger
