...
import { installChatbot, installIntegration, installRules } from "./commands/integrations.js";
...
export async function main(argv) {
  ...
  if ((cmd === "chat" || cmd === "chatbot") && (!subcmd || subcmd === "install" || subcmd === "add" || subcmd === "setup")) {
    const result = installChatbot([subcmd || "install", ...rest].filter(Boolean), process.cwd());
    console.log(JSON.stringify({ ok: true, command: "chat install", alias_used: cmd, ...result }, null, 2));
    return;
  }
